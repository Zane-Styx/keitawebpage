    ...
    function OpenSpeedtest() {
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "SendR") {
          ...
          htmlAnchorElement.innerHTML = circleSVG.innerHTML;
          circleSVG.innerHTML = dummyElement.innerHTML;
          if (typeof window.handleSpeedTestResults === "function") {
            try {
              window.handleSpeedTestResults({
                download: Number(downloadSpeed) || 0,
                upload: Number(uploadSpeed) || 0,
                ping: Number(pingEstimate) || 0,
                jitter: Number(jitterEstimate) || 0,
                downloadData: Number(dataUsedfordl) / 1048576 || 0,
                uploadData: Number(dataUsedforul) / 1048576 || 0
              });
            } catch (e) {
              console.error("Custom results handler failed:", e);
            }
          }
          if (!window.OST_DISABLE_RESULT_REDIRECT) {
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          if (auth == 6) {
            try {
              ...
              console.error('Response received:', return_data.substring(0, 200));
              // Use default server list if parsing fails
